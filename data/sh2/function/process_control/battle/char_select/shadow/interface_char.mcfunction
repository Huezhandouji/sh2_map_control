...
execute unless entity @a[scores={char_selected=1}] run tellraw @s [{"text":"§e[红RED]","hover_event":{"action":"show_text","value":"§a这个角色还没有被选择,你可以点击来选择"},"click_event":{"action":"run_command","command":"trigger trigger_shadow_select_char set 1"}}]
execute if entity @a[scores={char_selected=1},tag=!self] run tellraw @s [{"text":"§7红RED §c已被选择"}]
execute if entity @a[scores={char_selected=1},tag=self] run tellraw @s [{"text":"§a红RED §a你已选择"}]
