...
execute unless items entity @s hotbar.0 piglin_head run item replace entity @s hotbar.0 with piglin_head[consumable={"animation":"none","consume_seconds":0,"has_consume_particles":false,"sound":{"sound_id":""}},custom_data={"use_event":{"shadow_deploy_select":true}},item_name="§6选择部署角色"]
...
execute as @s[scores={trigger_shadow_select_char=0..}] run function sh2:process_control/battle/char_select/shadow_interface
