...
execute if score a calculate matches 0 run tellraw @s [{"text":"§a你正在升级前线影庭..还有"},{"score":{"objective":"calculate","name":"b"}},{"text":"§a秒"}]
scoreboard players reset a calculate
scoreboard players reset b calculate
scoreboard players reset num calculate

execute if score @s build_countdown matches 0 run function sh2:process_control/battle/sec3/point/succeed_upgrade_front_umbracore
...
execute positioned -461.5 6 -47.5 unless entity @s[distance=..10] run scoreboard players set @s build_countdown -1
execute positioned -461.5 6 -47.5 unless entity @s[distance=..10] run tellraw @s [{"text":"§c建造中断!在建造时必须处于据点内"}]
