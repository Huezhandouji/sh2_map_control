...
#如果换成别的物品就取消瞄准
execute unless items entity @s weapon.mainhand diamond[custom_data={"use_event":{"kovaci_gun":true}}] if score @s kovaci.is_aimming matches 1 run function sh2:char/kovaci/main_weapon/switch_aimming
#换弹
...
#子弹
execute as @e[type=block_display,tag=kovaci_bullet] at @s run function sh2:char/kovaci/main_weapon/fire/bullet_movement
#减少设计间隔计数
scoreboard players remove @s[scores={kovaci.shoot_interval=1..}] kovaci.shoot_interval 1
#替换物品
execute if entity @s[scores={kovaci.reload_cd=-1,faint_time=-1}] run return run function sh2:char/kovaci/main_weapon/item/available
